...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an AI assistant specializing in analyzing accident details from video footage.

  Based on the following information, provide a detection accuracy score (a float between 0 and 1).

  Time: {{{time}}}
  Location: {{{location}}}
  Description: {{{description}}}

  Generate a JSON object with just the accuracy.`,
});
